---
name: methodology-dispatching-parallel-agents
description: Best practices and patterns for Dispatching Parallel Agents
---
...
```markdown
Test failures:
- src/auth/login.test.ts (3 failures) â†’ Auth domain
- src/cart/checkout.test.ts (2 failures) â†’ Cart domain
- src/user/profile.test.ts (1 failure) â†’ User domain
...
If all NO â†’ Parallel is safe
If any YES â†’ Sequential or combined approach
```
